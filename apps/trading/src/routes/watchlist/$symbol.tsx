...
export const Route = createFileRoute('/watchlist/$symbol')({
  beforeLoad: async ({ context, params }) => {
    const symbol = await context.queryClient.ensureQueryData(
      getSymbolQueryOptions(params.symbol),
    );
...
function WatchlistSymbolPage() {
  ...
  const { data: watchlistSymbol } = useSuspenseQuery(
    getSymbolQueryOptions(symbolParam),
  );

  useSetLocalStorage(
    'watchlist:last-selected-symbol',
    watchlistSymbol?.symbol ?? null,
  );
...
function CoreSection({
  ...
  const { data: core } = useSuspenseQuery(
    getSymbolCoreDataQueryOptions(convex, symbol),
  );

  const bars = useMemo(() => toMarketBars(core.aggregates), [core.aggregates]);
  const quote = useMemo(
    () => previousCloseToQuote(core.previousClose),
    [core.previousClose],
  );
...
function IndicatorsSection({ symbol }: { symbol: string }) {
  ...
  const { data: indicators } = useSuspenseQuery(
    getSymbolIndicatorsQueryOptions(convex, symbol),
  );
...
function CorporateSection({ symbol }: { symbol: string }) {
  const convex = useConvex();
  const { data: corporate } = useSuspenseQuery(
    getSymbolCorporateDataQueryOptions(convex, symbol),
  );
  ...
  return (
    <>
      <div className='grid gap-10 py-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,20rem)]'>
        ...
        />
        <RelatedCompaniesCard
          relatedCompanies={corporate.relatedCompanies}
        />
      </div>
