import { getListResults, type RelatedCompanyResult } from '#/components/watchlist/market-data';
...
export function RelatedCompaniesCard({ relatedCompanies }: RelatedCompaniesCardProps) {
  const rows = getListResults<RelatedCompanyResult>(relatedCompanies).filter(
    (row): row is { ticker: string } => typeof row.ticker === 'string',
  );

  return (
    <Section eyebrow="Related companies" description="Peers based on market activity">
      {!rows.length ? (
        <p className="text-sm text-muted-foreground">No related companies available.</p>
      ) : (
