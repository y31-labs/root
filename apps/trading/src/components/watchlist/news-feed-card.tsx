...
export function NewsFeedCard({ symbol, news }: NewsFeedCardProps) {
  ...
  return (
    <Section eyebrow="Latest news" description={`Recent headlines tagged with ${symbol}`}>
      {!items.length ? (
        <p className="text-sm text-muted-foreground">No news available for this symbol.</p>
      ) : (
